...
import Image from "next/image";
import { sanityclient } from "@/sanity/lib/sainty";
...
const OurProducts = () => {
  ...
  useEffect(() => {
    sanityclient.fetch(allProductsQuery).then((data) => setProducts(data));
  }, []);
    ...
    <div className="relative w-full max-w-5xl mx-auto p-4 md:p-10">
      ...
      <div className="mt-8 grid grid-cols-2 gap-8 m-4 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4">
        {products.map((product) => (
          <div key={product._id} className="relative group w-full md:w-[220px] h-96 md:h-[400px] overflow-hidden rounded-md">
            <Image 
